function [freqs,sigfreqs,sigpowers,Amps,SigAmps]=lombscargle2(inputdata,dupe_elim,hifac,ofac)         %%% aik-eklemeler yapildi 11/04/2007
...
global cal calamp calfreq creationdate dataid datamatrix effm ep fhi freqs hifac info jmax lines n ...
	nmax nout np nuhifac ofac prob px py reg regnum s sigfreqs sigpowers tmax tmin win wintype x y  variance Amps SigAmps  %%% aik-eklemeler yapildi 11/04/2007
...
if ~isempty(freqs)   %%% aik-Power spectrum cizimi iptal  -aik  22/04/2007
...
% 	%CREATE RECONSTRUCTED WAVEFORM PROFILE FIGURE  %%% -aik  22/04/2007
% 	if ~isempty(sigfreqs)
...
if ~isempty(freqs)                                                       %%% aik-eklemeler yapildi 11/04/2007
    % Tüm genliklerin ve Significant genliklerin hesaplanmasi            %%% aik-eklemeler yapildi 11/04/2007
    Amps=2.0*(sqrt(freqs(:,2).*variance*n)/n);
    SigAmps=2.0*(sqrt(sigpowers.*variance*n)/n);
end %if ~isempty(freqs)
